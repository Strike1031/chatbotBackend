...
os.environ['OPENAI_API_KEY'] = "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"
...
# Create a dictionary to store user conversations
user_conversations = {}

def initialize_user_conversation(user_id):
    user_conversations[user_id] = ConversationBufferWindowMemory(
...
def initialize_agent_for_user(user_id):
    return initialize_agent(
        agent=f'chat-conversational-react-description_{user_id}',
        tools=tools,
        llm=llm,
        verbose=True,
        max_iterations=3,
...
def get_response(user_id, text):
    try:
        agent = initialize_agent_for_user(user_id)
        response = agent(chat_prompt.format_prompt(input=text).to_string())['output']
        return response
